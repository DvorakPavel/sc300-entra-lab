# Task 1 – Tenant foundation verification

**Date:**  
2026-03-30

**Tenant domain:**  
*******.onmicrosoft.com

## Goal
Verify access to the Microsoft 365 Developer sandbox and confirm the tenant foundation for the SC-300 lab.

## What I did
- Opened the subscription from the Microsoft 365 Developer dashboard.
- Signed in with the sandbox admin account.
- Verified access to the Microsoft 365 admin center.
- Verified access to the Microsoft Entra admin center.
- Confirmed tenant name, primary domain, and tenant ID.

## Result
The sandbox tenant was confirmed as active and ready for the next SC-300 lab tasks.

## Lessons learned
- The sandbox uses a separate cloud-only admin identity.
- Confirming tenant identity early helps avoid working in the wrong directory.
